...
public class PhoneBook {
  ...
  private void processQuery(Query query) {
    switch (query.type) {
      case "add":
        Contact contactToAdd = new Contact(query.name, query.number);
        if (map.containsKey(query.number)) {
          map.replace(query.number, contactToAdd);
        } else {
          map.put(query.number, contactToAdd);
        }
        break;
      case "del":
      ...
      default:
        ...
        System.out.println(map.get(number) == null ? "not found" : map.get(number).name);
        break;
    }
  }
